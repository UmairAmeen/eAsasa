<?php
namespace App\External\PdfInvoice;
/*******************************************************************************
* PHP Invoice                                                                  *
*                                                                              *
* Version: 1.5	                                                               *
...
class PhpInvoice extends FPDF_rotation  {
    ...
    public function setBarcode($path){
        $this->barcode = $path;
    }
    ...
    public function render($name='',$destination='',$check) {
        $this->AddPage();
        $this->Body($check);
        $this->AliasNbPages();
    ...
    public function Body($check) {
        ...
        if($this->items) {
            foreach($this->items as $key=> $item)
            {
                ...
                $this->Cell(5,$cHeight,$key+1,1,0,'L',1);
                $this->Cell($this->firstColumnWidth-5,$cHeight,iconv("UTF-8", "ISO-8859-1//TRANSLIT//IGNORE",$item['item']),1,0,'L',1);
                if($item['description']) {
                    $resetX = $this->GetX();
                    $resetY = $this->GetY();
                ...
                $this->Cell($width_other,$cHeight,$item['quantity'],1,0,'C',1);
                $this->Cell($this->columnSpacing,$cHeight,'',0,0,'L',0);
                ...
                $this->Cell($this->columnSpacing,$cHeight,'',1,0,'L',0);
                if($check == 1){
                    $this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252//TRANSLIT//IGNORE','-'),1,0,'C',1);
                }
                else{
                    if($this->curreny_direction == "left") {
                        $this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252//TRANSLIT//IGNORE', $this->currency.' '.number_format($item['price'],2,$this->referenceformat[0],$this->referenceformat[1])),1,0,'C',1);
                    }else{
                        $this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252//TRANSLIT//IGNORE', number_format($item['price'],2,$this->referenceformat[0],$this->referenceformat[1]).' '.$this->currency),1,0,'C',1);
                    }
                }
                if ($check == 1) {
                    $this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252//TRANSLIT//IGNORE','-'),1,0,'C',1);
                ...
                if(isset($this->discountField) and $this->hide_discount == 0)
                {
                    ...
                    {
                        $this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252//TRANSLIT//IGNORE',$item['discount']),1,0,'C',1);
                    }
                    else
                    {
                        $this->Cell($width_other,$cHeight,'',1,0,'C',1);
                    }
                }
                $this->Cell($this->columnSpacing,$cHeight,'',1,0,'L',0);

                // if($this->curreny_direction == "left") {
                //     $this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252//TRANSLIT//IGNORE', $this->currency.' '.number_format($item['total'],2,$this->referenceformat[0],$this->referenceformat[1])),1,0,'C',1);
                // }else{
                //     $this->Cell($width_other,$cHeight,iconv('UTF-8', 'windows-1252//TRANSLIT//IGNORE', number_format($item['total'],2,$this->referenceformat[0],$this->referenceformat[1]).' '.$this->currency),1,0,'C',1);
                // }
    ...
    public function Header() {
        ...
        if(!empty($this->time)){
            ...
            $this->Cell(32,$lineheight,iconv("UTF-8", "ISO-8859-1//TRANSLIT//IGNORE",'').'',0,0,'L');
            $this->SetTextColor(50,50,50);
            $this->SetFont($this->font,'',9);
        ...
        // $barcode = "daa";//\DNS2D::getBarcodeSVG("4445645656", "DATAMATRIX");
        // $barcode = str_replace("<", "&lt;", $barcode);
        // $barcode = str_replace(">", "&gt;", $barcode);
        // $this->WriteHTML(0, $lineheight,  $barcode, 0,1,'R');
        ...
        if($this->PageNo()== 1) {
            ...
            if($this->display_tofrom === true) {
                ...
                for($i=1; $i<max(count($this->from),count($this->to)); $i++) {
                    $this->Cell($width,$lineheight,iconv("UTF-8", "ISO-8859-1//TRANSLIT//IGNORE",$this->to[$i]),0,0,'L');
