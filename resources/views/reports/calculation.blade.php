...
<div class="col-md-12 text-center">
  ...
  @endif
  <h4>Report Uptil {{($to) ? (date(session('settings.misc.date_format','d-M-Y'), strtotime($to))) : date(session('settings.misc.date_format','d-M-Y'))}}</h4>
...
<br>
<h3>Business Capital Calculation Estimated Details - Product</h3>
...
<table class="table datatable" style="display: none">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{number_format($trans['purchase_quantity'])}}</td>
      <td>{{number_format($trans['purchase_quantity'] - $trans['sale_quantity'])}}</td>
  ...
  <tfoot>    
    <tr>
      <td>Total</td>
      <td></td>
      <td>{{number_format($invoice_quantity)}}</td>
      <td></td>
      <td><b>{{number_format($invoice_total)}}</b></td>
      <td></td>
...
@php($profit_percent = ($invoice_total == 0) ? 0 : (($balance - $totalDiscount - $expense) / $invoice_total) * 100)
      <tr>
          ...
          <td><strong>
              {{ number_format(abs($profit_percent),2) }}% {{ (($balance - $expense - $totalDiscount) > 0) ? " PROFIT" : " Invested" }}
            </strong></td>
        </tr>
    </tbody>
  </table>
