...
@endsection
@section('header')
    <title>Return Purchase Invoice Reporting</title>
    <link rel="stylesheet" type="text/css" href="{{ asset('assets/css/buttons.dataTables.min.css') }}">
    <div class="col-md-12 text-center">
        <h3>Return Purchase Invoice Reporting</h3>
...
@section('content')
    <div class="row showback">
        ...
        <form method="GET" action="{{ url('supplier_reporting') }}/returnPurchase_invoice">
                ...
                <div class="col-md-2">
                    <div class="form-group">
                        ...
                        <select name="is_return" id="is_return" class="form-control">
                            <option class="form-control" disabled>Select Type</option>
                            <option class="form-control" value="1" @if($is_return == 1) selected @endif>All Purchases</option>
                            <option class="form-control" value="0" @if($is_return == 0) selected @endif>Return invoices</option>
                        </select>
                    </div>
                </div>
                <div class="col-md-5">
        ...
        <div class="col-md-12">
            ...
            <table class="table datatable">
                ...
                <tbody>
                    ...
                    <?php $balance = 0; ?>
                    @foreach ($returnPurchase_invoice as $trans)
                        <?php $balance += $trans->invoiceTotal; ?>
                        <tr>
                            <td><a target="_blank"
                                    href="{{ url('invoices') }}/{{ $trans->invoice_idd }}">{{ $trans->invoice_idd }}</a>
                            </td>
                            <td>{{ $trans->bill_number }}</td>
                            <td>{{ date('d-M-Y', strtotime($trans->invoiceDate)) }}</td>
                            <td>{{ $trans->pPhone }}</td>
                            <td>{{ $trans->pAddress }}</td>
                            <td>{!! $trans->pName !!}</td>
                            <td>{{ $trans->notes }}</td>
                            <?php
                            ...
                            ?>
                            <td>{{ $trans->prod_quantity < 0 ? '(' . abs($trans->prod_quantity) . ')' : $trans->prod_quantity }}
                            </td>
                            <td>{{ $trans->total < 0 ? '(' . (abs($trans->total) + 0) . ')' : $trans->total + 0 }}</td>
                            <?php
                            ...
                            ?>
                            <td>{{ $trans->balance < 0? 'Debit (' . abs($trans->balance) . ')': ($trans->balance != 0? 'Credit ' . $trans->balance: $trans->balance) }}
                            </td>
                        </tr>
                    @endforeach
    ...
    <script type="text/javascript">
        $(document).ready(function(e) {
            ...
            var messageTopContent =
                'Day Sale Report date: {{ date_format_app($from) }} to  {{ date_format_app($to) }}';
