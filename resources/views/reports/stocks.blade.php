...
<div class="row showback">
	<form method="GET" action="{{url('reports')}}/department">
			 <div class="col-md-6">
...
<table class="table datatable">
  ...
  <tbody>
   ...
   ?>
    ...
    <?php 
    $balance[($stock->warehouse->name)?:'Other'] += $stock->quantity; 
    $amount[($stock->warehouse->name)?:'Other'] += $stock->quantity * $stock->sale->salePrice;  
    ?>
    <tr>
      <td>{{date('d-M-Y',strtotime($stock->date))}}</td>
      <td>{{$stock->product->name." ".$stock->product->brand." ".($size_enable!==false ? $stock->product->size : '') }}</td>
      <td>{{$stock->warehouse->name}} {{$stock->warehouse->code}}</td>
      <td>{{number_format($stock->quantity)}} {{($stock->sale->unit)?$stock->sale->unit->name:''}}</td>
      <td>{{ number_format($stock->quantity * $stock->sale->salePrice) }}</td>
      <td>{{number_format(array_sum($amount)) }}</td>
    </tr>
    @endforeach
  </tbody>
    <tfoot>      
    <tr>
      ...
      <td colspan="2" align="center"><h4>{{number_format(array_sum($amount))}}</h4></td>
    </tr>
    </tfoot>
</table>
...
<table class="table datatable">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{ number_format($amount[$key]) }}</td>
    </tr>
    @endforeach
  </tbody>
</table>    
