...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-plus"></i> Update Purchase </h1>
        <h3><a href="{{ url('purchases') }}">Back to All Purchases</a></h3>
    </div>
@endsection
@section('content')
    @include('error')
    <form id="product_add_form" method="post" action="{{ route('purchases.update', $invoice->id) }}">
        ...
        <div class="row">
                        ...
                        <div class="col-md-12" style="background-color: cornsilk;">
                            <div class="col-md-3">
                                <div class="form-group" id="date_form_group">
                                    <label>Date:</label>
                                    <input type="date" name="date" value="{{ $invoice->date }}" class="form-control"
                                        required="required">
                            ...
                            <div class="col-md-6">
                                <div class="form-group" id="customer_form_group">
                                    ...
                                    {!! Form::select('customer', $suppliers->pluck('name', 'id')->toArray(), $invoice->supplier_id, ['class' => 'form-control customer', 'required' => 'required']) !!}
                                    <small id="emailHelp" class="form-text text-muted">This is required</small>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <tr>
                                            <td>
                                                @if ($order->quantity > 0)
                                                    <input type="hidden" name="stype[]"
                                                        value="purchase"><span>purchase</span>
                                                @elseif($order->stock_purchase && $order->quantity < 0) <input
                                                        type="hidden" name="stype[]" value="refund"><span>refund</span>
                                                    @else
                                                        <input type="hidden" name="stype[]" value="damage"><span>Damage
                                                            Return</span>
                                                @endif
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <select name="warehouse[]" class="form-control">
                                                    <option value="" disabled="disabled">--Not Selected --</option>
                                                    @foreach ($warehouses as $wareh)
                                                        <option value="{{ $wareh->id }}"
                                                            {{ is_selected($wareh->id, $order->stock_purchase->warehouse_id) }}>
                                                            {{ $wareh->name }}</option>
                                                    @endforeach
                                                </select>
                                                <!-- <input type="hidden" name="warehouse[]" value="{{ $order->stock_purchase->warehouse_id }}"><span>{!! $order->stock_purchase ? $order->stock_purchase->warehouse->name : ' - <small><i>Stock is not stored</i></small>' !!}</span> -->
                                            </td>
                                            ...
                                            @else
                                                <td><input type="number" name="quantity[]" class="form-control"
                                                        value="{{ floatval($order->quantity) }}"></td>
                                            @endif
                                            <td><input type="number" step="0.01" min="0" name="minSalePrice[]"
                                                    class="form-control" placeholder="Sale Price"
                                                    value="{{ number_format($order->product->min_sale_price, 2, '.', '') }}">
                                            </td>

                                            @if (session()->get('settings.products.enable_advance_fields'))
                                                <td><input type="number" step="0.01" min="0" name="s_price[]"
                                                        class="form-control" placeholder="Sale Price"
                                                        value="{{ number_format($order->product->salePrice, 2, '.', '') }}">
                                                </td>
                                            @endif
                                            ...
                                            <td>
                                                <input type="number" step="0.01" min="0" name="sale_price[]"
                                                    ...
                                                    value="{{ number_format($order->salePrice, 2, '.', '') }}">
                                            </td>
                                            <td><span
                                                    class="sum">{{ $order->quantity * $order->salePrice }}</span>
                                            </td>
                                            <td><a data-id="'+product_id+'" data-qty="'+qty+'" data-wh="'+warehouse_id+'"
                                                    style="cursor: pointer;" onclick="clean(event, this)"><span
                            ...
                            <div class="col-md-8">
                                <div class="form-group">
                                    <p>
                                        ...
                                        <strong>Total Items: <span
                                                id="item_count">{{ count($invoice->orders) }}</span></strong>
                                    </p>
                                    <label>Notes:</label>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control " id="shipping" autocomplete="off" onkeyup="adjustTotal()"
                                        type="text" name="shipping" value="{{ $invoice->shipping }}">
                                </div>
                                <div class="form-group">
                                    <label>Discount:</label>
                                    <input class="form-control " id="discount" autocomplete="off" onkeyup="adjustTotal()"
                                        type="text" name="discount" value="{{ $invoice->discount }}">
                                </div>
                                <div class="form-group">
            ...
            <div class="col-md-12" style="margin-top: 10px;margin-bottom: 10px;">
                <center>{!! get_invoice_submit_buttons('Purchase', 'Update') !!}</center>
            </div>
        </div>
    </form>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).on("keyup", "input[name^=quantity],input[name^=sale_price],input[name^=purchase_price]", function(e) {
            adjustTotal();
        });
        var products_json_d = [];
        var current_bill = [];
        ...
        $(document).ready(function() {
            ...
            // $(".customer").val("{{ $invoice->customer_id }}").trigger("change");
        ...
        function keyDown(e, me) {
            ...
            if (keyCode == 9 || keyCode == 13) {
                ...
                var v = '<td><input type="hidden" name="stype[]" value="' + type + '"><span>' + type +
                    ...
                    qty +
                    '"><td><input type="number" step="0.01" min="0" name="minSalePrice[]" class="form-control" placeholder="Minimum Sale Price" value="' +
                    min_SalePrice +
                    '"></td>@if (session()->get('settings.products.enable_advance_fields'))<td><input type="number" step="0.01" min="0" name="s_price[]" class="form-control" placeholder="Sale Price" value="'+s_price+'"></td>@endif</td><td><input type="number" min="0" step="0.01" name="sale_price[]" class="form-control" placeholder="Purchase Price" value="' +
