...
@php  
    ...
    if($is_purchase != 1 && $enable_amount_in_words == 1)
    {
        $custom_width = "40%";
    }
@endphp
<table class="table" border="1" cellpadding="6" width="97%">
    ...
    @if(!env('RAZA'))
    <tr>
        <td width="10%"><strong>Total Qty</strong></td>
        <td width="{{ $custom_width }}">{{ $summary['totalQuantity'] }}</td>
        @if($is_purchase != 1 && $enable_amount_in_words == 1)
        <td width="10%"><strong>Amount in Words</strong></td>
                ...
                <td colspan="2" width="60%">
                    <table border="0">
                        <tr class="header-height">
                            <td>Payment Type:
                                <strong>{{ !empty($transaction->payment_type) ? ucfirst($transaction->payment_type) : '' }}</strong>
                            </td>
                        </tr>
                        @php
                            $bank_details = null;
            ...
            <td @if(!env('RAZA')) colspan="3" width="40%" @else width="100%" @endif >
                <table id="invoiceTotal" border="0">
                    @foreach ($summary_detail as $label => $value)
                        @if($value || $label == 'Balance')
                        <tr class="header-height">
                            <td><strong class="total_font_size">{{ $label }}</strong></td>
                            <td align="right"><strong class="total_font_size">{{ number_format($value) }}</strong></td> 
                        </tr>
                        @endif
                    @endforeach
                </table>
