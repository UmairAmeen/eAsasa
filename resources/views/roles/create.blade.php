...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-plus"></i> Add Role </h1>
    </div>
@endsection
...
@section('content')
                                ...
                                <ul>
                                    @foreach ($permission as $p)
                                        <li style="color: green;font-size:20px">
                                            <label>{{ Form::checkbox('permission[]', $p->id, in_array($p->id, $permission) ? true : false, ['class' => "name {$class} {$p->module}"]) }}&nbsp;{{ $p->display_name }}
                                            </label>
                                        </li>
                                    @endforeach
                                </ul>
    ...
    <div class="well well-sm">
        ...
        <a class="btn btn-link pull-right" href="{{ route('roles.index') }}"><i class="glyphicon glyphicon-backward"></i>
            Back</a>
    </div>
    {!! Form::close() !!}
@endsection
